...
doc_domain = "https://tillich-briefe.acdh.oeaw.ac.at"
entity_domain = "https://tillich.acdh.oeaw.ac.at"
files = sorted(glob.glob("./data/editions/*.xml"))
words_data = []
docs_data = []
for x in sorted(files):
    doc_id = f"<{doc_domain}/{os.path.split(x)[-1]}> "
    doc = TeiReader(x)
    ...
    for y in doc.any_xpath(".//tei:rs[@ref]"):
        ...
        ent_uri = f"<{entity_domain}/{check_for_hash(ent_id)}> "
        y.text = ent_uri
    text = extract_fulltext(
        doc.any_xpath(".//tei:body")[0], tag_blacklist=tag_blacklist
    )
...
words_df["entity"] = words_df.apply(
    lambda row: 1 if row["token"].startswith("<http") else row["entity"], axis=1
)
