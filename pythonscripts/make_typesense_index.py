...
for x in tqdm(files, total=len(files)):
    ...
    record["title"] = " ".join(
        " ".join(doc.any_xpath(".//tei:titleStmt/tei:title/text()")).split()
    )
    ...
    record["persons"] = [
        ...
        for x in doc.any_xpath(".//tei:back//tei:listPerson//tei:person//tei:persName")
    ]
    cfts_record["persons"] = record["persons"]
    record["places"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:back//tei:listPlace//tei:place//tei:placeName")
    ]
    cfts_record["places"] = record["places"]
    record["works"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:back//tei:listBibl//tei:bibl")
    ]
