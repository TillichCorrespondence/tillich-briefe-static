...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    as output
   -->
    <xsl:param name="relevant-uris" select="()"/>
    ...
    <xsl:template name="mam:idnosToLinks">
        ...
        <xsl:for-each select="$relevant-uris/descendant::item[not(@type)]">
            ...
            <xsl:if test="$idnos-of-current/descendant::tei:idno[@subtype = $abbr][1]">
                <xsl:variable name="current-idno" as="node()" select="$idnos-of-current/descendant::tei:idno[@subtype = $abbr][1]"/>
                <xsl:element name="a">
                    <xsl:choose>
                        <xsl:when test="$abbr = 'wikidata'">
                            <xsl:variable name="wikipediaVSdata" select="mam:wikidata2wikipedia($current-idno)" as="xs:string"/>
                        ...
                        <xsl:when test="$abbr = 'pmb'">
                            ...
                            <xsl:variable name="pmb-number" as="xs:string">
                                <xsl:choose>
                                    <xsl:when test="ends-with($current-idno, '/')">
                                        <xsl:value-of select="tokenize($current-idno, '/')[last() - 1]"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                            ...
                            <xsl:attribute name="href">
                                <xsl:value-of select="concat('https://pmb.acdh.oeaw.ac.at/apis/entities/entity/', $pmb-entitytype, '/', $pmb-number, '/detail')" />
                            </xsl:attribute>
                            <xsl:attribute name="target">
                                <xsl:text>_blank</xsl:text>
                            </xsl:attribute>
                    ...
                    <xsl:element name="span">
                        ...
                        <xsl:choose>
                            <xsl:when test="$abbr = 'wikidata'">
                                <xsl:variable name="wikipediaVSdata" select="mam:wikidata2wikipedia($current-idno)" as="xs:string"/>
                                <xsl:variable name="lang-code" select="substring(substring-after($wikipediaVSdata, 'https://'), 1, 2)"/>
                                <xsl:choose>
        ...
        <xsl:variable name="wikidata-entity">
            <xsl:choose>
                ...
                <xsl:when test="starts-with($wikidata-entry, 'https://www.wikidata.org/entity/')">
                    <xsl:value-of select="normalize-space(substring-after($wikidata-entry, 'https://www.wikidata.org/entity/'))" />
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="normalize-space(concat('Q', tokenize($wikidata-entry, 'Q')[last()]))" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="get-string" as="xs:string">
            <xsl:value-of select="concat('https://www.wikidata.org/w/api.php?action=wbgetentities&amp;format=xml&amp;props=sitelinks&amp;ids=', $wikidata-entity,'')" />
        </xsl:variable>
        <xsl:choose>
            <xsl:when test="document($get-string)/api[descendant::sitelink]/@success = '1'">
                <xsl:variable name="sitelinks" select="document($get-string)/descendant::sitelinks[1]" as="node()"/>
                <xsl:choose>
                    <xsl:when test="$sitelinks/sitelink[@site = 'dewiki']">
                        <xsl:value-of select="concat('https://de.wikipedia.org/wiki/', $sitelinks/sitelink[@site = 'dewiki']/@title)" />
                    </xsl:when>
                    <xsl:when test="$sitelinks/sitelink[@site = 'enwiki']">
                        <xsl:value-of select="concat('https://en.wikipedia.org/wiki/', $sitelinks/sitelink[@site = 'enwiki']/@title)" />
                    </xsl:when>
                    <xsl:when test="$sitelinks/sitelink[@site = 'frwiki']">
                        <xsl:value-of select="concat('https://fr.wikipedia.org/wiki/', $sitelinks/sitelink[@site = 'frwiki']/@title)" />
                    </xsl:when>
                    <xsl:when test="$sitelinks/sitelink[@site = 'itwiki']">
                        <xsl:value-of select="concat('https://it.wikipedia.org/wiki/', $sitelinks/sitelink[@site = 'frwiki']/@title)" />
                    </xsl:when>
                    <xsl:when test="$sitelinks/sitelink[@site = 'eswiki']">
                        <xsl:value-of select="concat('https://es.wikipedia.org/wiki/', $sitelinks/sitelink[@site = 'eswiki']/@title)" />
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:variable name="lang-code" select="substring($sitelinks/sitelink[not(@site='commonswiki')][1]/@site, 1, 2)"/>
                        <xsl:value-of select="concat('https://', $lang-code, '.wikipedia.org/wiki/', $sitelinks/sitelink[1]/@title)" />
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
