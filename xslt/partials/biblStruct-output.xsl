...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="mam:analytic-angabe">
        ...
        <xsl:choose>
            <xsl:when test="$analytic/tei:author[2]">
                <xsl:value-of select="mam:autor-rekursion($analytic, 1, count($analytic/tei:author))"/>
                <xsl:text>: </xsl:text>
            </xsl:when>
        ...
        <xsl:if test="$analytic/tei:editor[1]">
            ...
            <xsl:choose>
                <xsl:when test="$analytic/tei:editor[2]">
                    ...
                    <xsl:value-of select="mam:editor-rekursion($analytic, 1, count($analytic/tei:editor))"/>
                </xsl:when>
    ...
    <xsl:function name="mam:monogr-angabe">
        ...
        <xsl:if test="$monogr/tei:editor[1]">
            ...
            <xsl:choose>
                    ...
                    <xsl:for-each select="$monogr/tei:editor">
                        <xsl:choose>
                            <xsl:when test="contains(., ', ')">
                                <xsl:value-of select="concat(substring-after(normalize-space(.), ', '), ' ', substring-before(normalize-space(.), ', '))" />
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="."/>
                            </xsl:otherwise>
                ...
                <xsl:when test="contains($monogr/tei:editor, 'Hg.') or contains($monogr/tei:editor, 'Hrsg.') or contains($monogr/tei:editor, 'erausge') or contains($monogr/tei:editor, 'Edited')">
                ...
                <xsl:otherwise>
                    ...
                    <xsl:choose>
                        <xsl:when test="contains($monogr/tei:editor, ', ')">
                            <xsl:value-of select="concat(substring-after(normalize-space($monogr/tei:editor), ', '), ' ', substring-before(normalize-space($monogr/tei:editor), ', '))" />
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="$monogr/tei:editor"/>
                        </xsl:otherwise>
    ...
    <xsl:function name="mam:autor-rekursion">
        ...
        <xsl:if test="$autor-count &lt; $autor-count-gesamt">
            <xsl:text>, </xsl:text>
            <xsl:value-of select="mam:autor-rekursion($monogr, $autor-count + 1, $autor-count-gesamt)"/>
        </xsl:if>
    </xsl:function>
